# -*- coding: utf-8 -*-
"""
Created on Tue Aug 17 00:38:45 2021
...
def shuffleData(X, y):
    ...
    y = y[index]
    return X, y;
nb_lstm_outputs = 32    # 输出神经元个数
nb_time_steps =41   # 时间序列的长度
nb_input_vectors = 4 # 每个输入序列的向量维度
def creat_model():
    #scheduler = keras.callbacks.ReduceLROnPlateau(simple_adam, 'max', factor=0.5, patience=3)
    model = K.models.Sequential()
    
    #model.add(Bidirectional(LSTM(units=nb_lstm_outputs, input_shape=(nb_time_steps, nb_input_vectors),return_sequences=True)))
    model.add(Bidirectional(GRU(units=nb_lstm_outputs,return_sequences=True),input_shape=(nb_time_steps, nb_input_vectors)))
    model.add(Bidirectional(GRU(units=nb_lstm_outputs)))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="deep learning m6A analysis")
